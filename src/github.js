...
// === تعديل دالة listing لتكون عامة وبدون فلتر ===
export async function githubListFiles(env, folder) {
  ...
  const data = await res.json();
  // إزالة الفلتر الخاص بـ .log لجلب كل الملفات
  const files = data.filter(item => item.type === "file").map(item => ({ name: item.name, path: item.path, sha: item.sha, size: item.size, download_url: item.download_url }));
